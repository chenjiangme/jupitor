    ...
    var body: some View {
        VStack(spacing: 6) {
            ZStack {
                ...
                // Close gain marker (color by close fraction: red at low, green at high).
                if let cg = stats.closeGain, cg > 0 {
                    let cf = stats.high > stats.low ? min(max((stats.close - stats.low) / (stats.high - stats.low), 0), 1) : 0.5
